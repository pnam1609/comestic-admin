        ...
        <div className="modal-content">
          <div className="py-3 mb-20" >
            <h3 className="m-0 font-weight-bold text-primary" style={{ textAlign: 'center' }}>
              Các sản phẩm của đơn đặt hàng: <span className="text-danger">{item.orderSupplyId}</span>
            </h3>
          </div>
          {item.detailOSList.length === 0 ? "Đơn đặt hàng này không có phiếu nhập" :
            <>
              <table className="table table-bordered" id="dataTable" width="100%" cellSpacing="0">
                ...
                <tbody>
                  {item.detailOSList.map((sp, index) => {
                    return <tr key={index}>
                      <td>{sp.productId}</td>
                      <td>{sp.quantity}</td>
